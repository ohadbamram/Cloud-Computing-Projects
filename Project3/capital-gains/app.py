...
# URL for the Stock Service (Kubernetes Service name)
STOCK_SERVICE_URL = "http://stocks.we-love-sap.svc.cluster.local:8000/stocks" 
...
def capital_gains():
    query = request.args
    numsharesgt = query.get('numsharesgt')  # Filter for shares greater than
    numshareslt = query.get('numshareslt')  # Filter for shares less than

    total_gain = 0
    try:
        # Fetch all stocks from the Stock Service
        response = requests.get(f"{STOCK_SERVICE_URL}")
        response.raise_for_status()  # Raise an error for bad responses (4xx, 5xx)
        ...
        print(f"Error fetching stocks from Stock Service: {e}")
        return jsonify({"error": "Failed to fetch stocks from Stock Service"}), 500

    return str(total_gain)  # Return the total gain as JSON
